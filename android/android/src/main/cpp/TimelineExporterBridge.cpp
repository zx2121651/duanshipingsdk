...
Java_com_sdk_video_timeline_TimelineExporter_nativeExportAsync(JNIEnv *env, jobject thiz, jlong handle, jlong timelineHandle, jlong engineHandle, jobject onProgress, jobject onComplete) {
    ...
    // 实例化并配置离线导出的 DecoderPool，自动注册所有音视频与贴纸素材
    auto decoderPool = std::make_shared<DecoderPool>();
    for (int z = 0; z <= 255; ++z) {
        auto track = (*timelinePtr)->getTrack(z);
        if (!track) continue;
        for (const auto& clip : track->getAllClips()) {
            if (!clip) continue;
            auto type = clip->getType();
            if (type == Clip::MediaType::VIDEO || 
                type == Clip::MediaType::IMAGE || 
                type == Clip::MediaType::STICKER) {
                std::string clipId = clip->getId();
                std::string srcPath = clip->getSourcePath();
                if (!srcPath.empty()) {
                    decoderPool->registerMedia(clipId, srcPath);
                }
            }
        }
    }
    compositor->setDecoderPool(decoderPool);
