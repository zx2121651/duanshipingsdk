...
#include "ai/FaceLandmarkDetector.h"
#include "ai/TfliteInferenceEngine.h"
#include "ai/FaceReshapeFilter.h"
#include "ai/MakeupFilter.h"
#include "ai/HairSegmentationFilter.h"
#include "ai/BeautyFilter.h"
#include "EffectPlugin.h"
#include "OboeAudioEngine.h"
#include "ai/HandLandmarkDetector.h"
#include "ai/SegmentationFilter.h"
#include "ai/ChromaKeyFilter.h"
...
struct EngineWrapper {
    ...
    bool setupRecordingSurface(JNIEnv* env, jobject surface) {
        ...
        recordThread = std::thread([this, config]() {
            ...
            const char* vsrc = vsrc_str.empty() ? nullptr : vsrc_str.c_str();
            const char* fsrc = fsrc_str.empty() ? nullptr : fsrc_str.c_str();

            const char* DEFAULT_RECORD_VERT = R"(#version 300 es
layout(location=0) in vec4 position;
layout(location=1) in vec2 inputTextureCoordinate;
out vec2 vtc;
void main() {
    gl_Position = position;
    vtc = inputTextureCoordinate;
})";

            const char* DEFAULT_RECORD_FRAG = R"(#version 300 es
...
Java_com_sdk_video_RenderEngine_nativeInit(JNIEnv *env, jobject thiz, jobject assetManager) {
    ...
    // [FIX] Drain any residual GL errors from the entire C++ init chain
    // (sniffCapabilities, VAO/VBO, shader compile, FBO creation, pipeline compile).
    // This is the final barrier before returning to Kotlin where glGenTextures()
    // creates the OES texture for SurfaceTexture.  Any uncleared error here would
    // poison SurfaceTexture.updateTexImage() on the first camera frame.
    {
        GLenum err = glGetError();
        int drainCount = 0;
        while (err != GL_NO_ERROR && drainCount < 16) {
            __android_log_print(ANDROID_LOG_WARN, "NativeBridge",
                "nativeInit: drained stale GL error 0x%x (count=%d)", err, drainCount + 1);
            drainCount++;
            err = glGetError();
        }
    }
...
JNIEXPORT void JNICALL
Java_com_sdk_video_RenderEngine_nativeMarkFrameRendered(JNIEnv *env, jobject thiz, jlong handle, jint textureId) {
    EngineWrapper* wrapper = reinterpret_cast<EngineWrapper*>(handle);
    if (wrapper && wrapper->filterEngine) {
        wrapper->filterEngine->markFrameRendered(static_cast<uint32_t>(textureId));
    }
}
