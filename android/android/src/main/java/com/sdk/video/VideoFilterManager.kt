...
class VideoFilterManager(private val context: android.content.Context,
    ...
    var onSurfaceRecreated: ((Surface) -> Unit)? = null
    ...
    suspend fun initialize(): Result<Unit> {
        return runOnGLThread {
            val isRecreating = (_engineState.value == FilterEngineState.RUNNING || _engineState.value == FilterEngineState.DEGRADED)
            if (isRecreating) {
                Log.i("VideoFilterManager", "Re-initializing engine due to context restoration")
                renderEngine.onContextLost()
            }
            ...
            val res: Int
            if (renderEngine.getNativeHandle() == 0L) {
                res = renderEngine.init(context.assets)
            } else {
                res = renderEngine.onContextRestored()
                if (res == 0) {
                    renderEngine.recreateOESTexture()
                }
            }
