...
Result Filter::initialize() {
    ...
    // [FIX] Notify subclasses so they can cache their uniform locations.
    // Previously this was only called from updateShaderSource (hot-reload),
    // leaving OES2RGBFilter's textureMatrix/flip handles uninitialized.
    onProgramRecompiled();
