...
void GLStateManager::bindTexture(GLenum target, GLuint texture) {
    ...
    if (index >= 0 && index < 32) {
        if (target == GL_TEXTURE_2D) {
            // [FIX] Cross-invalidate the OES cache on this unit.  GL only
            // allows one texture target per unit; when we bind a 2D texture,
            // any previously-bound OES texture is implicitly unbound by the
            // driver.  If we don't zero m_boundTextureOES[index], a later
            // OES bind of the same ID would be skipped (cache hit), leaving
            // the unit still bound to the 2D texture → GL_INVALID_OPERATION.
#ifdef __ANDROID__
            m_boundTextureOES[index] = 0;
#endif
            ...
            // [FIX] Mirror: invalidate 2D cache when binding OES
            m_boundTexture2D[index] = 0;
