...
void VulkanShaderProgram::setUniform2f(const std::string& n, float x, float y) {
    float v[2] = {x, y};
    registerPushConstant(n, 8);
    auto it = m_pcOffsets.find(n);
    if (it != m_pcOffsets.end()) std::memcpy(m_pcData.data() + it->second, v, 8);
}
void VulkanShaderProgram::setUniform4f(const std::string& n, float x, float y, float z, float w) {
    float v[4] = {x, y, z, w};
    registerPushConstant(n, 16);
    auto it = m_pcOffsets.find(n);
    if (it != m_pcOffsets.end()) std::memcpy(m_pcData.data() + it->second, v, 16);
}
void VulkanShaderProgram::setUniformMat4(const std::string& n, const float* m) {
    registerPushConstant(n, 64);
    auto it = m_pcOffsets.find(n);
    if (it != m_pcOffsets.end()) std::memcpy(m_pcData.data() + it->second, m, 64);
}
